...
function OverlayCanvas({ persons, handRes, faceRes, W, H, showTrails, showHands, showFace, showDist, videoRef: vidRef }) {
  ...
  useEffect(() => {
    ...
    // El canvas tiene width/height CSS = 100%/100% del contenedor
    // Sincronizar sus atributos px con el tamaño CSS real en cada frame
    const sync = () => {
      ...
      persons.forEach((p) => {
        ...
        if (lSh && lEl && lWr) {
          ...
          if (ang != null) {
            ...
            ctx.fillText(ok ? "Codo OK" : ang < 80 ? "Codo cerrado" : "Codo abierto", lEl.x * cW, lEl.y * cH + 22);
            ctx.shadowBlur = 0;
          }
        }
        ...
        if (rSh && rEl && rWr) {
          ...
          if (ang != null) {
            ...
            ctx.fillStyle = ok ? T.green : T.red; ctx.font = "bold 11px 'DM Mono',monospace";
            ctx.textAlign = "center"; ctx.shadowColor = ok ? T.green : T.red; ctx.shadowBlur = 6;
            ctx.fillText(ok ? "Codo OK" : ang < 80 ? "Codo cerrado" : "Codo abierto", rEl.x * cW, rEl.y * cH + 22);
            ctx.shadowBlur = 0;
          }
        }
        ...
        if (showDist && faceRes?.multiFaceLandmarks?.[0]) {
          ...
          if (lm234 && lm454 && lm10 && lm152) {
            ...
            // Cara promedio adulto: ~14-15cm ancho, ~19-20cm alto
            ...
            if (distCm) {
              ...
              const distLabel = `Distancia pantalla: ~${Math.round(distCm)} cm`;
              const subLabel = distCm < 45 ? "Muy cerca — aleja el monitor" : distCm > 70 ? "Muy lejos — acerca el monitor" : "Distancia óptima (45-70 cm)";
              ...
              // Textos
              ctx.font = "bold 12px 'DM Mono',monospace"; ctx.fillStyle = distColor;
              ctx.textAlign = "left"; ctx.shadowColor = distColor; ctx.shadowBlur = 4;
              ctx.fillText(distLabel, bx + 10, by + 16);
              ctx.font = "10px 'DM Mono',monospace"; ctx.fillStyle = "rgba(255,255,255,0.7)"; ctx.shadowBlur = 0;
