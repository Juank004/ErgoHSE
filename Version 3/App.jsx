...
// Face oval — contorno completo hasta mentón
const FACE_OVAL = [10, 338, 297, 332, 284, 251, 389, 356, 454, 323, 361, 288, 397, 365, 379, 378, 400, 377, 152, 148, 176, 149, 150, 136, 172, 58, 132, 93, 234, 127, 162, 21, 54, 103, 67, 109, 10];
// Puntos clave SIMÉTRICOS: nariz, ojos (esquinas), boca (simétrica), mentón, pómulos, cejas
const FACE_KEY_PTS = [
  // Nariz
  1, 4, 5, 6,
  // Mentón y frente
  ...
  useEffect(() => {
    ...
    if (showFace && faceRes?.multiFaceLandmarks) {
      faceRes.multiFaceLandmarks.forEach(face => {
        if (!face || face.length < 100) return;
        // Óvalo facial completo hasta mentón
        ctx.shadowColor = "rgba(255,0,180,0.85)"; ctx.shadowBlur = 16;
        ctx.strokeStyle = "#FF00CC"; ctx.lineWidth = 2;
        ctx.beginPath();
        let faceStarted = false;
        FACE_OVAL.forEach((fi) => {
          const lm = face[fi]; if (!lm) return;
          const px = lm.x * cW, py = lm.y * cH;
          if (!faceStarted) { ctx.moveTo(px, py); faceStarted = true; }
          else ctx.lineTo(px, py);
        });
        ctx.closePath(); ctx.stroke();
        FACE_KEY_PTS.forEach(fi => {
          const lm = face[fi]; if (!lm) return;
          ctx.beginPath(); ctx.arc(lm.x * cW, lm.y * cH, 2.5, 0, Math.PI * 2);
          ctx.fillStyle = "#FF00CC"; ctx.shadowColor = "rgba(255,0,200,0.9)"; ctx.shadowBlur = 10; ctx.fill();
          ctx.beginPath(); ctx.arc(lm.x * cW, lm.y * cH, 1, 0, Math.PI * 2);
